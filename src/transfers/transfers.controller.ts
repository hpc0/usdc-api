...
import { ApiQuery, ApiOperation, ApiTags, ApiParam, ApiBody } from '@nestjs/swagger';
...
export class TransfersController {
    ...
    constructor(
        ...
        this.provider = new ethers.JsonRpcProvider(this.config.get<string>('ETH_RPC_URL'));

        this.usdc = new ethers.Contract(
            this.config.get<string>('USDC_CONTRACT') ?? '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48',
            usdcAbi,
            this.provider
        );
    }
    ...
    @ApiParam({ name: 'address', required: true, description: 'Ethereum address to query the balance of' })
    ...
    @ApiParam({ name: 'address', required: true, description: 'Ethereum address to fetch history for' })
    @ApiQuery({ name: 'limit', required: false, description: 'Maximum number of transfers to return', example: 20 })
    async getHistory(
